...
MDBX_CXX14_CONSTEXPR size_t check_length(size_t headroom, size_t payload);
MDBX_CXX14_CONSTEXPR size_t check_length(size_t headroom, size_t payload,
                                         size_t tailroom);
static MDBX_CXX17_CONSTEXPR size_t strlen(const char *c_str) noexcept;
...
template <typename T, typename A> struct move_assign_alloc<T, A, false> {
  ...
  static constexpr bool is_moveable(T *target, T &source) noexcept {
    return allocator_is_always_equal<A>() ||
...
struct default_capacity_policy {
  ...
  static MDBX_CXX20_CONSTEXPR size_t advise(const size_t current,
                                            const size_t wanna) {
    ...
    if (wanna > current)
       ...
       * and not less than pettiness_threshold */
      return (wanna +
              ::std::min(size_t(pettiness_threshold),
                         ::std::max(size_t(max_reserve), current)) +
              pettiness_threshold - 1) &
             ~size_t(pettiness_threshold);

    if (current - wanna >
        /* shrink if reserve is more than 1/2 or max_reserve,
         * but not less than pettiness_threshold */
        ::std::min(wanna + pettiness_threshold, size_t(max_reserve)))
      return (wanna + pettiness_threshold - 1) & ~size_t(pettiness_threshold);
...
template <class ALLOCATOR = ::mdbx::legacy_allocator,
          typename CAPACITY_POLICY = default_capacity_policy>
class buffer {
public:
  using allocator_type = rebind_alloc<ALLOCATOR, uint64_t>;
...
private:
  ...
  struct silo /* Empty Base Class Optimization */ : public allocator_type {
    ...
    allocate_storage(size_t bytes) {
      ...
      static_assert(unit > 1, "size of ALLOCATOR::value_type must be > 1");
    ...
    union bin {
      ...
      constexpr byte &lastbyte() noexcept { return inplace_[sizeof(bin) - 1]; }

      enum class make_mode { construct, assign };

      template <make_mode mode> constexpr byte *make_inplace() noexcept {
        if (mode == make_mode::assign && is_allocated())
      ...
      template <make_mode mode>
      constexpr byte *make_allocated(allocator_pointer ptr,
                                     size_t capacity_bytes) noexcept {
        MDBX_CONSTEXPR_ASSERT(
            (capacity_bytes & le_lastbyte_mask) == 0 &&
            ((capacity_bytes >>
              (sizeof(allocated_.capacity_bytes_) - 1) * CHAR_BIT) &
             be_lastbyte_mask) == 0);
        if (mode == make_mode::construct || !is_allocated())
      ...
      constexpr bin(size_t capacity_bytes = 0) noexcept {
        ...
        make_inplace<make_mode::construct>();
        (void)capacity_bytes;
      }
      MDBX_CXX20_CONSTEXPR ~bin() { make_inplace<make_mode::assign>(); }
      constexpr bin(allocator_pointer ptr, size_t capacity_bytes) noexcept {
        make_allocated<make_mode::construct>(ptr, capacity_bytes);
      }
      constexpr bin(bin &&ditto) noexcept {
        if (ditto.is_inplace()) {
          ...
          ditto.make_inplace<make_mode::assign>();
      ...
      constexpr bin &operator=(const bin &ditto) noexcept {
        if (ditto.is_inplace()) {
          ...
          make_allocated<make_mode::assign>(ditto.allocated_.ptr_,
      ...
      static MDBX_CXX20_CONSTEXPR size_t advise_capacity(const size_t current,
        ...
        return ::std::min(
            size_t(max_capacity),
            ::std::max(sizeof(bin) - 1,
                       reservation_policy::advise(current, wanna)));
      }
    ...
    MDBX_CXX20_CONSTEXPR void release() noexcept {
      if (bin_.is_allocated()) {
        ...
        bin_.template make_inplace<bin::make_mode::assign>();
      }
    }
    ...
    reshape(const size_t wanna_capacity, const size_t wanna_headroom,
      ...
      if (external_content) {
        byte *new_place;
        if (bin_.is_allocated()) {
          assert(bin::is_suitable_for_inplace(new_capacity));
          deallocate_storage(bin_.allocated_.ptr_,
                             bin_.allocated_.capacity_bytes_);
          new_place = bin_.template make_inplace<bin::make_mode::assign>();
        } else {
          assert(!bin::is_suitable_for_inplace(new_capacity));
          const auto pair = allocate_storage(new_capacity);
          assert(pair.second >= new_capacity);
          new_place = bin_.template make_allocated<bin::make_mode::construct>(
              pair.first, pair.second);
        }
        new_place += wanna_headroom;
        if (MDBX_LIKELY(length))
      ...
      if (bin_.is_allocated()) {
        ...
        byte *new_place = bin_.template make_inplace<bin::make_mode::assign>() +
                          wanna_headroom;
        if (MDBX_LIKELY(length))
          MDBX_CXX20_LIKELY memcpy(new_place, content, length);
        ...
        if (MDBX_LIKELY(length))
          MDBX_CXX20_LIKELY memcpy(new_place, content, length);
        return bin_.template make_allocated<bin::make_mode::construct>(
            pair.first, pair.second);
      }
    }
...
public:
  ...
  buffer(silo &&bin) noexcept
      : silo_(::std::move(bin)), slice_(silo_.data(), silo_.capacity()) {}
  ...
  buffer &assign(silo &&src) noexcept(move_assign_alloc::is_nothrow()) {
  ...
  buffer &operator=(silo &&src) noexcept(move_assign_alloc::is_nothrow()) {
    return assign(::std::move(src));
  }
